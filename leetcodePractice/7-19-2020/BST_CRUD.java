...
public TreeNode searchBST(TreeNode root, int val) {
    if (root == null) {
        return null;
    } // didn't find a node with value == val

    if (val < root.val) {
        return searchBST(root.left, val); // val less than root.val. Try to find it in left subtree
    } else if (val > root.val) {
        return searchBST(roo.right, val);
    } else {
        return root; // find it
    }
}
...
// 详情可以看 http://www.algolist.net/Data_structures/Binary_search_tree/Removal
